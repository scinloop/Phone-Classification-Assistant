...
def evaluate_one_image(image_array):
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            if max_index == 0:
                result = ('这是xiaomi的可能性为： %.6f' % prediction[:, 0])
            elif max_index == 1:
                result = ('这是OPPO的可能性为： %.6f' % prediction[:, 1])
            elif max_index == 2:
                result = ('这是VIVO的可能性为： %.6f' % prediction[:, 2])
            else:
                result = ('这是apple的可能性为： %.6f' % prediction[:, 3])
            return result
